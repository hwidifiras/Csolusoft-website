          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {processSteps.map((item, i) => (
              <div key={i} className="relative">
                <div className="text-6xl font-black text-primary-500/20 mb-4">{item.step}</div>
                <h3 className="text-xl font-bold mb-3">{item.title}</h3>
                <p className="text-slate-300 text-sm leading-relaxed">{item.description}</p>
                {i < processSteps.length - 1 && (
                  <div className="hidden lg:block absolute top-8 right-0 w-1/2 h-0.5 bg-gradient-to-r from-primary-500/50 to-transparent"></div>
                )}
              </div>
            ))}
          </div>
      ...
      <section className="py-24 bg-primary-500">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link 
              to={Page.Contact}
              className="px-8 py-4 bg-white text-primary-500 rounded-2xl font-bold hover:bg-primary-50 transition-all flex items-center justify-center space-x-2"
            >
            ...
            <Link 
              to={Page.About}
              className="px-8 py-4 bg-primary-400 text-white rounded-2xl font-bold hover:bg-primary-300 transition-all"
            >
              En savoir plus
            </Link>
          </div>
